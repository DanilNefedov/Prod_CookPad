...
import { Box, Collapse, Menu, MenuItem, Skeleton, useMediaQuery } from "@mui/material";
import IconButton from '@mui/material/IconButton';
import FavoriteIcon from '@mui/icons-material/Favorite';
import Link from "next/link";
import {
    arrowSwiper, boxTypeRecipe, cardBottom, containerMenu, containerMobileMenu, contentPostionAbsolute,
    cookBtn, desktopBotInfo, headerCard, headerMove, mainCard, mobileBottomInfo, mobileButtons,
    mobileMenu,
    typeRecipe
} from "@/app/(main)/home/styles";
...
export const CardContentBlock = memo(({ props }: { props: Props }) => {
    const { recipe_id, id, isSkeleton } = props
                        ...
                        {recipe?.media
                            ...
                            .map((el, index) => (
                                ...
                                <SwiperSlide key={el.media_id} className="media-main-slide" virtualIndex={index}>
                                    <SwiperMediaCard props={{ el, name: recipe?.name, priority: index < 4 }} />
                                </SwiperSlide>
                            )
                            )}
